<?php
	...
	//db_connect() is created to connect to the database
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "", "bookseller");
		if(!$conn){
	...
	//select4LatestBook() returns an array of 4 books to display on index page(book image and book isbn number)
	...
	function getAll($conn){
		$query = "SELECT * from books as bk join publisher as pb WHERE pb.publisherid = bk.publisherid ORDER BY bk.book_title;";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
